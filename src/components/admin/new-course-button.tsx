...
const BLANK = { code: "", title: "", dept: "CS", credits: "3", cap: "30", modality: "In-person", status: "active" };

export function NewCourseButton({ onAdd }: { onAdd?: (row: AdminCourseRow) => void } = {}) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement> | React.MouseEvent) {
    ...
    setTimeout(() => {
      ...
      onAdd?.({
        ...
        level:          (() => { const n = parseInt(form.code.match(/\d+/)?.[0] ?? "100"); return (Math.floor(Math.min(n, 500) / 100) * 100) as AdminCourseRow["level"]; })(),
        enrolled:       0,
      ...
      setTimeout(() => { setOpen(false); setForm({ ...BLANK }); setSave("idle"); }, 900);
    }, 1000);
  }

  function handleClose() {
    if (save === "saving") return;
    setOpen(false); setForm({ ...BLANK }); setSave("idle");
  }
    ...
    <>
      ...
      {open && (
        <>
          ...
          <div className="m-sheet" role="dialog" aria-modal="true" aria-label="New course">
            ...
            <form onSubmit={handleSubmit} className="m-sheet__body">
              <div className="m-fields-2">
                <label className="m-field">
                  ...
                  <input className="m-field__input m-mono" placeholder="e.g. CS-401"
                    value={form.code} onChange={(e) => set("code", e.target.value)} required />
                </label>
                <label className="m-field">
                  <span className="m-field__label">Department</span>
                  <select className="m-field__input m-field__select" value={form.dept}
              ...
              <label className="m-field">
                ...
                <input className="m-field__input" placeholder="e.g. Advanced Distributed Systems"
              ...
              <div className="m-fields-2">
                <label className="m-field">
                  ...
                  <select className="m-field__input m-field__select" value={form.credits}
              ...
              <div className="m-fields-2">
                <label className="m-field">
                  ...
                  <select className="m-field__input m-field__select" value={form.modality}
                ...
                <label className="m-field">
                  ...
                  <select className="m-field__input m-field__select" value={form.status}
            ...
            <div className="m-sheet__foot">
              <button className="m-btn m-btn--ghost" onClick={handleClose} disabled={save === "saving"}>Cancel</button>
              <button className="m-btn m-btn--primary" disabled={save !== "idle" || !form.code.trim() || !form.title.trim()}
                onClick={handleSubmit}>
                {save === "idle"   && <><PlusIcon size={13} /> Create course</>}
